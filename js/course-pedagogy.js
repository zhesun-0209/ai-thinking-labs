...
const notebooks = {
  ...
  "ch5-bfs": {
    ...
    misconception: "误区：BFS 总能找到最低代价路径——它保证最少边数，只有边权相同时也保证总代价最低。",
    selfCheck: "x 的三个邻居按什么顺序入队？为何路径只有2条边，却不止展开2个节点？",
  ...
  "ch5-minimax": {
    ...
    misconception: "误区：MiniMax 就是普通路径搜索——它评估博弈树，对手会主动选对你最差的应对。",
  ...
  "ch6-forward": {
    ...
    misconception: "误区：前向链一定比后向链更正确——二者驱动方向不同，结论是否可靠都依赖事实与规则；规则不完备还可能漏掉结论。",
  ...
  "ch6-pathrank": {
    ...
    selfCheck: "本例两个候选都得3分，按约定谁排在前面？这能证明它是唯一代表作吗？",
  ...
  "ch8-attention": {
    ...
    misconception: "误区：注意力权重就是下一词概率——它决定如何加权输入信息，词元概率还需后续输出层计算。",
  ...
  "ch7-metrics": {
    ...
    mentalModel: "门槛提高会减少判正样本、可能增加漏检；精确率和 F1 是否改善要实际计算。",
  ...
  "ch9-selfattn": {
    ...
    mustSee: ["Q/K/V 各从哪来", "Softmax 后权重和为 1", "「写」这一行如何关注「鲁迅」"],
  ...
  "ch9-w2v": {
    ...
    mentalModel: "用共现学习中心词与上下文的匹配分数；语境相似的词可能学到相似表示。",
  ...
  "ch11-mdp": {
    ...
    mustSee: ["智能体→环境→r,s′ 环", "累积回报 G 如何算", "γ 如何影响未来奖励的权重"],
    misconception: "误区：不折扣未来奖励就会短视——γ=0 才只看眼前；γ 越接近1越重视未来，有限回合可以取1。",
  ...
  "ch11-actor": {
    ...
    mustSee: ["π(a|s) 分布", "V(s) 基线", "用 δ=r+γV(s′)−V(s) 估计优势"],
  ...
  "ch11-td": {
    ...
    misconception: "误区：TD 总比蒙特卡洛更准——TD 用估计值自举，通常方差较低但可能有偏，两者需按任务取舍。",
    selfCheck: "若 γ=0，TD 更新退化成什么？",
    whenToUse: "在线强化学习、大状态空间、需要价值自举的场景。",
  },
  "ch11-epsilon": {
    wish: "ε-贪心 = 保留随机探索机会，避免只利用当前估计最好的动作；随机时也可能选中最优动作。",
