...
const chapters = {
  ...
  8: {
    ...
    pillars: [
      ...
      { label: "对齐", desc: "Attention：按相关性加权取信息" },
    ],
    case: "血糖 MLP + 鲁迅三元组 + 翻译对齐",
  },
  9: {
    wish: "语言 = 分词→向量→上下文→生成；重点把各算法放进同一条 pipeline，而不是孤立记忆术语。",
    pillars: [
      { label: "Pipeline", desc: "BPE → 向量 → Self-Attn → LM" },
  ...
  10: {
    ...
    pillars: [
      { label: "Inductive bias", desc: "局部性 vs 全局注意力" },
      { label: "自监督", desc: "MAE 遮罩重构、CLIP 对比对齐" },
      { label: "统一积木", desc: "ViT 与 NLP 共享 Transformer" },
    ],
    case: "4×4 小图标",
  },
  11: {
    wish: "行动 = 在不确定环境里试错学习；重点理解 Agent–Env 环、探索与利用、TD 自举，以及策略与价值的分工。",
    pillars: [
      { label: "交互环", desc: "s → a → r,s′ 循环" },
      { label: "长期回报", desc: "γ 折扣未来奖励" },
      { label: "探索", desc: "ε-贪心：不能只会 exploitation" },
    ],
    case: "订机票四状态 MDP",
  },
  12: {
...
const notebooks = {
  ...
  "ch8-forward": {
    ...
    misconception: "误区：以为前向只算一次——推理时算一遍，训练时每个 batch 都要重算并保存 a 供反向用。",
  ...
  "ch8-attention": {
    wish: "注意力 = 按需求从 Encoder 里「软查询」信息，解决对齐与长依赖。",
    mentalModel: "图书馆检索：Q 是你的问题，K 是索引，V 是内容，权重是相关性。",
    mustSee: ["Encoder-Decoder 桥的位置", "「日记」行对「写」列权重最高", "Softmax 后权重和为 1"],
    misconception: "误区：Attention 是 RNN 的简单替代——它并行看全句，但 O(n²) 且需位置信息。",
    selfCheck: "解码「日记」时，Q 来自 Decoder 还是 Encoder？",
  ...
  "ch7-tree": {
    ...
    mentalModel: "二十Questions 游戏：每个问题把人群分成更纯的两堆。",
  ...
  "ch9-selfattn": {
    wish: "Self-Attention 让每个词「看见」全句，并行聚合上下文。",
    mentalModel: "开班会：每人根据与发言者的相关度决定听谁。",
    mustSee: ["Q/K/V 各从哪来", "Softmax 后权重和为 1", "「写」行如何 attend「鲁」"],
    misconception: "误区：Attention 自带位置感——需额外加位置编码。",
    selfCheck: "Self-Attention 与 Encoder-Decoder Attention 的 Q 来源有何不同？",
  ...
  "ch9-w2v": {
    ...
    whenToUse: "词级语义、冷启动 embedding、理解 Attention 之前的表示层。",
  },
  "ch9-lm": {
    wish: "语言模型 = 自回归链式概率，训练学 P(w_i | 上文)。",
    mentalModel: "接龙游戏：给定前缀，预测下一个最合理的 token。",
  ...
  "ch10-vit": {
    wish: "ViT = 把图像当句子：patch 是 token，Transformer 看全局。",
    mentalModel: "切蛋糕成块，每块当一个词，用 Self-Attention 看全图。",
    mustSee: ["patch 如何切分与展平", "位置编码为何必要", "Encoder 与 NLP 同构"],
    misconception: "误区：ViT 完全取代 CNN——小数据上 CNN 仍常更强；ViT 需大数据预训练。",
    selfCheck: "没有位置编码，patch 顺序丢失会怎样？",
    whenToUse: "大规模预训练、需长程依赖的图像任务。",
  },
  "ch10-mae": {
    wish: "MAE = 遮大部分 patch，Encoder 学压缩表示，Decoder 学重构。",
    mentalModel: "完形填空：只看 25% 词猜全文，逼模型学语义而非死记。",
    mustSee: ["75% patch 被遮", "Encoder 输入仅可见 patch", "重构像素作为自监督信号"],
    misconception: "误区：MAE 在做分类——预训练阶段无标签，下游才微调分类头。",
    selfCheck: "为何遮 75% 而不是 50%？（更高 mask 比 → 更难任务）",
  ...
  "ch11-mdp": {
    ...
    mustSee: ["Agent→Env→r,s′ 环", "累积回报 G 如何算", "γ 为何小于 1"],
  ...
  "ch11-epsilon": {
    wish: "ε-贪心 = 训练时故意随机试差动作，避免 exploitation 陷阱。",
  ...
  "ch12-mcts": {
    ...
    mustSee: ["Selection 沿 UCT 下降", "Expansion 添新叶", "Backup 更新 Q/N"],
  ...
  "ch12-repr": {
    ...
    mustSee: ["代数表征 loss 卡住", "退火允许差解", "换几何后 loss 下降"],
